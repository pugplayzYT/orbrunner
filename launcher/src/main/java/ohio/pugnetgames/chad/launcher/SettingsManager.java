 ...
 * Also watches the file for changes made by the running game process.
 */
public class SettingsManager {
    ...
    // Settings with defaults matching the game's hardcoded defaults
    ...
    /** Watches the settings file for changes written by the game. Callback runs on the watcher thread. */
                    ...
                    while (watching) {
                        ...
                        for (WatchEvent<?> event : key.pollEvents()) {
                            ...
                            if ("settings.dat".equals(changed.getFileName().toString())) {
                                Thread.sleep(60); // wait for write to finish
                                load();
